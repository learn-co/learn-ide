...
class PopoutEmulator {
  ...
  attach() {
    ...
    document.body.style.fontSize = `${localStore.get('learn-ide:currentFontSize')}px`
  ...
  setFontSize(sizeInt) {
    ...
    // the first call to fit is responding to the size change
    this.emulator.fit()
    // the second call to fit is fitting the emulator in the window
    this.emulator.fit()
  }
}
