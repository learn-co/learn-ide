...
const helper = {
  ...
  buildCSS({foreground, background, ansiColors, fontFamily}) {
    var context = '.terminal.xterm'

    var css = `${context} {color: ${foreground}; background-color: ${background}}\
               .bottom .tool-panel .terminal-resizer, ${context} .xterm-rows, ${context} .xterm-viewport {background-color: ${background}}\n`;

    if (fontFamily.length) {
      css += `${context} {font-family: ${fontFamily}, courier-new, courier, monospace}\n`
    }

    ansiColors.forEach((color, index) => {
      css += `${context} .xterm-color-${index} {color: ${color}}\
							${context} .xterm-bg-color-${index} {background-color: ${color}}\n`
    });
...
const colors = {
  ...
  getCSS() {
    ...
    var fontFamily = atom.config.get(`${name}.fontFamily`);

    return helper.buildCSS({foreground, background, ansiColors, fontFamily});
  },
