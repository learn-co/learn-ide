...
function isCoreOrPackageStackTrace(rawStack) {
  return rawStack.some((entry) => fs.isAbsolute(entry.getFileName()))
};
...
export default function({message, url, line, originalError, preventDefault}) {
  ...
  if (originalError.name === 'BufferedProcessError') {
    ...
    if (originalError.stack && !isCoreOrPackageStackTrace(originalError.getRawStack())) {
      return;
    }
