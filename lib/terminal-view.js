...
class TerminalView extends View {
  ...
  resizeAfterDrag({pageY, which}) {
    ...
    var limit = 100,
        ...
        availableHeight = currentHeight + this.offset().top,
        proposedHeight = availableHeight - pageY;

    if (proposedHeight < limit && proposedHeight < currentHeight) {
      return
    }

    if (pageY < limit) {
      proposedHeight = availableHeight - limit
    }
  ...
  fit() {
    ...
    var $emulator = $(this.emulator.element),
        rowHeight = this.emulator.viewport.currentRowHeight,
        rowCount  = this.emulator.rows,
        newHeight = $emulator.height() || (rowCount * rowHeight);
