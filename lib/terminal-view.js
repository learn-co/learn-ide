...
class TerminalView extends View {
  ...
  subscribe() {
    this.emulator.attachCustomKeydownHandler(e => {
      return this.catchSaveShortcutOnWindowsOrLinux(e);
    })

    this.emulator.attachCustomKeydownHandler(e => {
      if (this.isAttemptToScroll(e)) {
        e.preventDefault()
        return false
      }
    })
  ...
  catchSaveShortcutOnWindowsOrLinux(e) {
    // TODO: if this returned false, it likely would not need to dispatch its own command
    var {which, ctrlKey} = e;
  ...
  isAttemptToScroll({which, ctrlKey, metaKey}) {
    var isUpOrDown = (which === 38) || (which === 40);
    var cmdUpOrDown = metaKey && isUpOrDown && (process.platform === 'darwin');
    var ctrlUpOrDown = ctrlKey && isUpOrDown && (process.platform !== 'darwin');

    return cmdUpOrDown || ctrlUpOrDown
  }
