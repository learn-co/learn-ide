...
class TerminalView extends View {
  ...
  forceFontSize(size) {
    ...
    this.forcedStyle = atom.styles.addStyleSheet(css, {priority: 1});
  }
  ...
  fit() {
    ...
    var newHeight = $(this.emulator.rowContainer).height();

    if (!newHeight) {
      newHeight = this.emulator.viewport.currentRowHeight * this.emulator.rows;
    }
